      ...
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="form-group">
            ...
            <input autoFocus placeholder="e.g. Website Redesign" value={form.name}
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            {loading ? 'Loading…' : `${projects.length} project${projects.length !== 1 ? 's' : ''} you're part of`}
          </p>
        </div>
        {isAdmin && (
        ...
        <div className="empty-state">
          ...
          {isAdmin && (
            <button className="btn-primary" style={{ marginTop: 20 }} onClick={() => setShowModal(true)}>
              <Plus size={16} style={{ marginRight: 6 }} /> Create Project
            </button>
          )}
        </div>
      ) : (
          ...
          {projects.map(p => (
            <div key={p.id} className="project-card" onClick={() => navigate(`/projects/${p.id}`)}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                <div style={{ width: 40, height: 40, background: projectColor(p.id), borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: 'var(--text)', fontWeight: 700 }}>
                  {p.name[0]?.toUpperCase()}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span className={`badge badge-${p.my_role}`}>{p.my_role}</span>
                  {isAdmin && (
                    <button className="btn-danger btn-sm" onClick={e => deleteProject(e, p.id)} title="Delete project">
                      <Trash2 size={13} />
                    </button>
                  )}
                </div>
      ...
      {showModal && (
        <CreateProjectModal
          ...
          onCreated={p => setProjects(prev => [{ ...p, member_count: 1, task_count: 0, my_role: 'admin' }, ...prev])}
        />
      )}
    </div>
  );
